...
/*** Added for picstroom ***/
-(BOOL)loadFileSync:(NSString *)path destinationPath:(NSString *)destinationPath {
    NSString* fullPath=[NSString stringWithFormat:@"/files/%@%@", root, path];
    NSURLRequest* urlRequest=[self requestWithProtocol:kDBProtocolHTTPS host:kDBDropboxAPIContentHost path:fullPath parameters:nil];
    
    ASIHTTPRequest *request = [ASIHTTPRequest requestWithURL:urlRequest.URL];
...
-(DBMetadata*)loadMetadataSync:(NSString*)path withHash:(NSString*)hash {
    ...
    [self requestWithProtocol:kDBProtocolHTTPS host:kDBDropboxAPIHost path:fullPath parameters:params];
...
- (void)loadThumbnail:(NSString *)path ofSize:(NSString *)size intoPath:(NSString *)destinationPath 
{
    ...
    if(size) {
        params = [NSDictionary dictionaryWithObjectsAndKeys: size, @"size", @"PNG", @"format", nil];
    }
...
DBErrorCode addFileUploadToRequest(NSMutableURLRequest *urlRequest, NSString *filename, NSString *sourcePath)
{
    ...
    //*** changed due to leak described here http://forums.dropbox.com/topic.php?id=23290&replies=9
    urlRequest.HTTPBody = [NSData dataWithContentsOfFile:tempFilePath];
